...
import { E, onRender } from '../utility'
...
function Header({ history }: { history: any }) {
	...
	const hideAccountPop = () => {
		const t = E('web3-status-connected')
		const tx = E('account_popup')
		if (tx != null) {
			t.click()
		}
	}
	...
	useEffect(() => {
		if (account) {
			localStorage.setItem('network', nativeCurrency.symbol)
		}
	})
	return (
		<HeaderFrame onClick={hideAccountPop}>
			<HeaderRow isDark={isDark}>
				<Title href=".">
					<DXswapIcon>
						<img width={200} src={isDark ? Logo : LogoDark} alt="logo" />
					</DXswapIcon>
				</Title>
				<HeaderLinks>
					...
					<StyledNavLink
						id={`stake-nav-link`}
						to={'/stake'}
						isActive={() => history.location.pathname.includes('/stake')}
					>
						{t('Stake')}
					</StyledNavLink>
					<StyledNavLink
						id={`stake-nav-link`}
						to={'/limit'}
						isActive={() => history.location.pathname.includes('/limit')}
					>
						{t('Limit')}
					</StyledNavLink>
					<StyledNavLink
					...
					<StyledNavLink
						...
						to={'/vote'}
						isActive={() =>
							history.location.pathname.includes('/vote') || history.location.pathname.includes('/voting')
						}
					>
						{t('Vote')}
					</StyledNavLink>
					{/* <StyledNavLink
						id={`airdrop-nav-link`}
						to={'/airdrop'}
						isActive={() => history.location.pathname.includes('/airdrop')}
					>
						{t('Airdrop')}
					</StyledNavLink> */}
			...
			<HeaderControls>
				<HeaderElement>
					<AccountElement active={!!account} style={{ pointerEvents: 'visible' }} networkError={!!error}>
