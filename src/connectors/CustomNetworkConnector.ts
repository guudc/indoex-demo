...
//this just extends the error class and then adds a public code and public data variables
...
class CustomMiniRpcProvider implements AsyncSendable {
  ...
  public readonly request = async (
    ...
    if (!response.ok) console.log(`${response.status}: ${response.statusText}`);//throw new RequestError(`${response.status}: ${response.statusText}`, -32000)
    const body = await response.json()
    if ('error' in body) {
      console.log(body?.error?.message, body?.error?.code, body?.error?.data)
    } else if ('result' in body) {
      return body.result
    } else {
      console.log(`Received unexpected JSON-RPC response to ${method} request.`, -32000, body)
    }
  }
}
